...
class PersonaManager:
    ...
    def create_persona(self, data: PersonaCreate) -> EstadoResponse:
        ...
        self.logger.info(f"Verificando existencia previa de la persona ID: {data.id}")
        ...
        self.logger.info(f"Persona creada y familia actualizada correctamente: {data.id}")
    ...
    def update_persona(self, id_persona: str, data: PersonaUpdate) -> EstadoResponse:
        self.logger.info(f"Iniciando actualización de persona con ID: {id_persona}")
    ...
    def delete_persona(self, id_persona: str) -> EstadoResponse:
        self.logger.info(f"Iniciando eliminación lógica de persona con ID: {id_persona}")
        persona: Persona = self.persona_repository.get(id_persona)
        if not persona:
            self.logger.error(f"Persona no encontrada con ID: {id_persona}")
            raise AppException("Esa persona no está registrada")
        ...
        self.logger.info(f"Actualizando familia {familia.id} al eliminar persona")
        self.familia_repository.update(familia.id, familia)
    ...
    def get_personas(self, page: int, page_size: int) -> PaginatedPersonas:
        self.logger.info(f"Obteniendo personas: página {page}, tamaño {page_size}")
    ...
    def assing_familia_persona(self, data: AssingFamilia) -> AsignacionFamiliaResponse:
        self.logger.info(f"Asignando personas a familia con ID: {data.familia_id}")
        ...
        for persona_id in data.personas_id:
            ...
            self.logger.info(f"Persona asignada a familia {data.familia_id}: {persona_id}")
    ...
    def _update_familia_members(self, id_familia_old: int, id_familia_new: int):
        if id_familia_new is not None and id_familia_old != id_familia_new:
            self.logger.info(f"Actualizando miembros entre familias: {id_familia_old} -> {id_familia_new}")
            ...
            if id_familia_old is None:
                ...
                self.logger.info(f"Agregando miembro a nueva familia ID: {id_familia_new}")
            ...
            self.logger.info(f"Actualizando familia original ID: {id_familia_old}")
            self.familia_repository.update(id_familia_old, familia_old)

            self.logger.info(f"Actualizando nueva familia ID: {id_familia_new}")
            self.familia_repository.update(id_familia_new, familia_new)

            self.logger.info(f"Actualización de familias completada: {id_familia_old} -> {id_familia_new}")
